...
describe('NominalExpand : used to display nominal', () => {
	...
	it(`Input 11333777.88 result "11.333.777,88"`, () => {
		const nominal = new NominalExpand('11333777.88');
		expect(nominal.toSparator().value).toBe('11.333.777,88');
	});

	it(`Input 11333777,88 result ""`, () => {
		const nominal = new NominalExpand('11333777,88');
		expect(nominal.toSparator().value).toBe('');
	});

	it(`Input 11333777,88 result "" | switchSeparator to .`, () => {
		const nominal = new NominalExpand('');
		expect(nominal.switchSeparator({ to: '.' }).toSparator().value).toBe('');
	});

	it(`Input 11333777,88 result "11.333.777,88"`, () => {
		const nominal = new NominalExpand('11333777,88');
		expect(nominal.switchSeparator({ to: ',' }).toSparator().value).toBe('11.333.777,88');
	});
	...
	it(`Input Rp11.333.777,885 result "Rp11.333.777,88" | prefix:Rp | toFixed:2`, () => {
		...
		expect(
			nominal
				.cleanFrom({ search: 'Rp' })
				.removeSparator({ separator: '.' })
				.switchSeparator({ to: ',' })
				.toSparator({ toFixed: 2 })
